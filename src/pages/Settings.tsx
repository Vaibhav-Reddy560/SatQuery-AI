...
export default function Settings() {
  ...
  const [ai, setAi] = useState<{ configured: boolean; source: "backend" | "browser" | "offline"; model: string }>({
    configured: false,
    source: "offline",
    model: "gemini",
  });

  // Ask the AI service where it can run (backend key → browser key → offline).
  useEffect(() => {
    let mounted = true;
    aiStatus().then((s) => mounted && setAi(s));
    return () => {
      mounted = false;
    };
  }, []);
        ...
        <div className="divide-y divide-border-subtle">
          <Row
            ...
            description={
              ai.configured
                ? ai.source === "backend"
                  ? "Answers are generated by the Gemini model through the SatQuery backend (key held server-side)."
                  : "Answers are generated directly from your browser with VITE_GEMINI_API_KEY (dev mode — don't use in production)."
                : "No Gemini API key is configured, so the assistants answer locally from page data and the query page falls back to its deterministic demo engine."
            }
          >
            <span
              className={cn(
                "font-mono text-mono-sm",
                ai.configured ? "text-phosphor" : "text-text-muted"
              )}
            >
              {ai.configured ? `Live · ${ai.model}` : "Offline"}
            </span>
          </Row>
          <Row
            title="API key"
            description="Recommended: set GOOGLE_API_KEY (backend .env) so the key stays server-side. For a browser-only demo you can set VITE_GEMINI_API_KEY in the frontend .env instead — never use a paid key this way."
          >
            <span
              className={cn(
                "font-mono text-mono-sm",
                ai.configured ? "text-text-muted" : "text-warning"
              )}
            >
