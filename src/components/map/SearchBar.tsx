...
export function SearchBar({ onLocationSelect, className }: SearchBarProps) {
  ...
  return (
    <div className={cn("relative", className)}>
      <div className="flex items-center gap-2 px-3 py-2 bg-bg-secondary/90 border border-border-default rounded-lg backdrop-blur-sm focus-within:border-accent transition-colors">
        {loading ? (
        ...
        <input
          ...
          className="bg-transparent text-sm text-text-primary placeholder:text-text-muted outline-none flex-1 min-w-0"
      ...
      {open && results.length > 0 && (
        <div className="absolute top-full left-0 right-0 mt-1 bg-bg-secondary border border-border-default rounded-lg shadow-xl z-20 max-h-60 overflow-y-auto">
          {results.map((r, i) => (
            <button
              key={i}
              onClick={() => handleSelect(r)}
              className="flex items-start gap-2 w-full px-3 py-2.5 text-left hover:bg-bg-hover transition-colors"
            >
              <MapPin className="h-4 w-4 text-accent shrink-0 mt-0.5" />
              <span className="text-sm text-text-secondary line-clamp-2">{r.label}</span>
            </button>
          ))}
        </div>
      )}
